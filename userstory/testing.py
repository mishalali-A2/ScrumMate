...
class N8NWorkflowManager:
    ...
    def trigger_workflow_auto(self, workflow_id, data=None, max_wait_seconds=300, poll_interval=1):
        """
        Auto-detect webhook path or fallback to /run.
        WAIT: if a webhook node exists, poll the computed webhook URL until it's responsive,
        then send the payload exactly like the original script.
        """
        ...
        if webhook_node:
            ...
            # === OVERRIDE: send to the exact URL the user specified ===
            full_url = "http://localhost:5678/webhook-test/create_board_from_us"
            print(f"ℹ Workflow uses webhook. Will wait for listener then call {method} {full_url}")

            # --- WAIT FOR LISTENER (poll) ---
            start = time.time()
            while True:
                try:
                    # Try a lightweight request to check readiness.
                    # Use OPTIONS if supported, otherwise GET (no body).
                    try:
                        probe = requests.options(full_url, timeout=5)
                    except Exception:
                        probe = requests.get(full_url, timeout=5)
                    # Consider endpoint ready if we get any HTTP response with status < 500
                    if probe is not None and hasattr(probe, "status_code") and probe.status_code < 500:
                        print(f"ℹ Detected listener at {full_url} (status {probe.status_code}). Sending payload now.")
                        break
                except Exception:
                ...
                if max_wait_seconds is not None and elapsed >= max_wait_seconds:
                    print(f"❌ Timed out waiting for listener at {full_url} after {max_wait_seconds} seconds.")
                    return False
                time.sleep(poll_interval)

            # After listener detected, perform the same request as original script
            try:
                if method == "GET":
...
if __name__ == "__main__":
    ...
    # default: wait up to 300 seconds, polling every 1s; pass different values if desired
    success = manager.trigger_workflow_auto(WORKFLOW_ID, data=payload, max_wait_seconds=300, poll_interval=1)
